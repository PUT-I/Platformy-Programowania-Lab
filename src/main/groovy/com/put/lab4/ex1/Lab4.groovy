...
class Lab4 extends Application {
    ...
    static void ex1() {
        Chart chartA = FunctionChart.createChart(-1, 1, FunctionChart.AREA_CHART) {
            double x -> return 0
        }
        Chart chartB = FunctionChart.createLineChart(-4, 4) {
            double x -> return -(x**2)
        }
        Chart chartC = FunctionChart.createChart(-4, 4, FunctionChart.POINT_CHART) {
            double x -> return -(x**2) - x + 3
        }
        Chart chartD = FunctionChart.createChart(-20, 20, FunctionChart.LINE_CHART) {
            double x -> return Math.sin(x) * x * Math.atan(x)
        }
